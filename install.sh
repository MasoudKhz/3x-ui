...
install_x-ui() {
    ...
    if [ $# == 0 ]; then
        last_version="v1.7.1"
        #$(curl -Ls "https://api.github.com/repos/MHSanaei/3x-ui/releases/latest" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
        ...
        wget -N --no-check-certificate -O /usr/local/x-ui-linux-$(arch3xui).tar.gz https://github.com/MasoudKhz/3x-ui/archive/refs/tags/v1.7.1.zip
        if [[ $? -ne 0 ]]; then
